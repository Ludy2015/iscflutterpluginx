...
@interface IscPlayerView ()<HVPVoiceIntercomClientDelegate>
...
@property (nonatomic, strong) HVPVoiceIntercomClient *intercomClient;    // 语音对讲实例对象
...
-(void)onMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result{
    ...
    if ([[call method] isEqualToString:@"startRealPlay"]) {
        ...
        result([self startVoiceTalk:url]);
        
    } if ([[call method] isEqualToString:@"stopVoiceTalk"]) {          // 关闭语音对讲
        
        result([self stopVoiceTalk]);
...
#pragma mark - 视频相关操作
/// 开启语音对讲
- (NSDictionary *)startVoiceTalk:(NSString *)url {
    
    if (self.isTalkPlaying) {
        [self stopVoiceTalk];
    }
    BOOL result = [self.intercomClient startVoiceIntercom:url];
    return @{@"ret" : @(result),
             @"msg" : @""
    };
}
...
/// 关闭语音对讲
- (NSDictionary *)stopVoiceTalk {
    BOOL isStopTalking = NO;
    NSError * error;
    [self.intercomClient stopVoiceIntercom:&error];
    if (!error) {
        isStopTalking = YES;
...
- (NSDictionary *)enableSound:(BOOL)enable {
    BOOL result = [self.player enableSound:enable error:nil];
    return @{
        @"ret" : @(result),
        @"msg" : @""
    };
...
- (void)player:(HVPPlayer *)player playStatus:(HVPPlayStatus)playStatus errorCode:(HVPErrorCode)errorCode {
    ...
    switch (playStatus) {
        case HVPPlayStatusSuccess: {
            ...
            // 默认关闭声音
            [self.player enableSound:NO error:nil];
        }
            break;
...
//            message = @"开启对讲成功";
            ...
            _mTalkStatus = [NSNumber numberWithInt:SUCCESS];
        } else if (playStatus == HVPPlayStatusFailure){
            
            _mTalkStatus = [NSNumber numberWithInt:FAILED];
            // 关闭语音对讲
            if (self.isTalkPlaying) {
                [self.intercomClient stopVoiceIntercom:nil];
            }
//            if (errorCode == HVPErrorCodeURLInvalid) {
//                message = @"URL输入错误请检查URL或者URL已失效请更换URL";
//            } else {
...
//            }
            ...
            _mTalkStatus = [NSNumber numberWithInt:EXCEPTION];
            // 关闭语音对讲
            if (self.isTalkPlaying) {
                [self.intercomClient stopVoiceIntercom:nil];
            }
//            message = [NSString stringWithFormat:@"语音对讲异常%@, 错误码", @(errorCode)];
        ...
        NSDictionary * ret = @{@"status" : _mTalkStatus};
